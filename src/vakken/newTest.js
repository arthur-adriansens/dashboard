...
window.onload = function() {
    ...
    //let action = getAction("monday", 11, 16);
...
function getAction(day, hour, minute) {
    ...
    //frans maandag:  actions[day].frans; OF actions[day]["frans"];
        //start frans (maan)dag: actions[day]["frans"].start;
        //by index:  var todaysAction = Object.values(actions[day])[1].start;
    console.log("today:", today);
   //console.log("begin "+todayKeys[3]+": "+todayValues[3].start);
    ...
    while (JSON.stringify([currentHour, currentMinute]) != JSON.stringify([hour, minute])) { 
        ...
        if (JSON.stringify(starts[i]) == JSON.stringify([currentHour, currentMinute])) { //https://stackoverflow.com/questions/30820611/why-doesnt-equality-check-work-with-arrays
            ...
            } else if (typeof nextAction == "undefined") {
                nextAction = "no next action";
            }
...
function getInfo(action, nextAction) {
    var time = getTime();
    var current = actions[time[0]][action]; // actions["monday"][action]
    var next = actions[time[0]][nextAction];
    document.getElementById("part1").innerHTML = "U heeft " + Object.keys(actions["monday"]).length + " activiteiten op " + time[0] + ".";
    document.getElementById("part2").innerHTML = "Momenteel heeft u " + action + " van " + current.start[0]+"u"+current.start[1] + " tot " + current.end[0] + "u"+current.end[1] + " in lokaal " + current.classroom + " met " + current.teacher+".";
    if (nextAction != "no next action") {document.getElementById("part3").innerHTML = "Daarna heeft u " + nextAction + " van " + next.start[0]+"u"+next.start[1] + " tot " + next.end[0]+"u"+next.end[1] + " in lokaal " + next.classroom + " met " + next.teacher + "."}
}

function getTime() {
    var date = new Date();
    if (date.getDay() == 0 || date.getDay() == 6) {
        if (date.getDay()==0) {var day = "sunday", nextDay = "monday"}
        if (date.getDay()==6) {var nextDay = "sunday"}
    } else {
        var day = days[date.getDay()], nextDay = days[date.getDay()+1];
    }
